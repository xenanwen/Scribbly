...
**Live demo:** [Vercel link](https://scribbly-inmx5intb-xenanwens-projects.vercel.app)
...
2. **SQL Editor → New query** → build the schema. There are two routes to the same
   end state:

   **A fresh project — one file.** Run
   [`supabase/schema-full.sql`](supabase/schema-full.sql): the entire database as it
   stands, every table, policy, grant, trigger and function, already including the two
   invite fixes below.

   **An existing database — the numbered migrations, in this order.** Each is
   idempotent, so re-running one is safe; the order matters, because each replaces
   policies and functions the previous one created:
   ...
   5. [`supabase/005_invite_token_fix.sql`](supabase/005_invite_token_fix.sql) —
      ...
      `extensions` schema where the invite functions cannot see it
...
> **Never** put the `service_role` key (or an `sb_secret_` key) in `.env.local` or anywhere
> in `src/`. It bypasses Row Level Security, and anything prefixed `VITE_` is compiled into
> the public JS bundle. `src/lib/supabase.ts` throws at startup if it detects one.

#### If something in the database looks wrong

Three read-only queries, none of which change anything:

| File | Answers |
| --- | --- |
| [`supabase/verify.sql`](supabase/verify.sql) | Is RLS on, are the grants right, are there orphaned rows, does every board have an owner? |
| [`supabase/sharecheck.sql`](supabase/sharecheck.sql) | Which object the Share feature needs is missing — every table, function, signature and grant, one row each |
| [`supabase/statecheck.sql`](supabase/statecheck.sql) | Did a stray re-run of `schema.sql` restore the old single-user policies? |

A fourth, [`supabase/audit-fake-members.sql`](supabase/audit-fake-members.sql), lists any
account-less "invented" people left over from before `003`, and what deleting each would
take with it.
...
│   ├── Primitives.tsx  Stateless shared pieces: avatars, chips, buttons, icons
│   ├── Popover.tsx     Anchored menu for the filters (hand-rolled, no floating-ui)
│   └── Overlay.tsx     Modal + Drawer shells (focus trap, Escape)
└── styles/
    ├── index.css       Import order — tokens first, everything else consumes them
...
```
supabase/
├── schema.sql              Base DDL, then 002 → 005 on top (or schema-full.sql alone)
├── schema-full.sql         The whole database in one file, for a fresh project
├── verify.sql              PASS/FAIL on RLS, grants, orphans, board ownership
├── sharecheck.sql          Every object the Share feature needs, present or MISSING
├── statecheck.sql          Whether an old schema.sql re-run reverted the policies
└── audit-fake-members.sql  Account-less members left over from before 003
tests/
├── run.mjs                 esbuild strips the types, node runs the result
├── drag.test.ts            Fractional positions, cross-column drops, rebalancing
├── logic.test.ts           Ordering, filters, stats, due-date urgency, member access
├── sharing.test.ts         Invite URLs, token round-trips, roles
└── validate.test.ts        Email + password rules
```
...
The token is 32 random bytes (~256 bits) generated by `gen_invite_token()` in
...
That randomness comes from `gen_random_uuid()`, not from pgcrypto's
`gen_random_bytes()`. Supabase installs extensions into the `extensions` schema,
while every function here pins `search_path = public` — so pgcrypto is invisible
from inside them, and the original version failed with `42883 function
gen_random_bytes(integer) does not exist` the moment anyone clicked *Create link*.
`gen_random_uuid()` is core Postgres in `pg_catalog`, reachable whatever the
search path. See
[`supabase/005_invite_token_fix.sql`](supabase/005_invite_token_fix.sql).
...
[`supabase/003_real_members.sql`](supabase/003_real_members.sql), and two fixes to the
invite functions in [`004`](supabase/004_invite_param_fix.sql) and
[`005`](supabase/005_invite_token_fix.sql). All of it together, for a fresh project, is
[`supabase/schema-full.sql`](supabase/schema-full.sql).
...
**Once the repo is connected, every push to `main` redeploys** — there is no separate
deploy step. Worth knowing what that does *not* cover: the schema lives in Supabase, not
in the bundle, so a migration only takes effect when it is run in the SQL editor of
whichever project the production build points at. Check that project under
**Vercel → Settings → Environment Variables** before assuming a fix has shipped.
